/*
 * ESP32 HC-SR04 with Ultrasonic.h + WiFi + MQTT
 * GPIO 4 (Trig) / GPIO 5 (Echo)
 * 
 * 需要安裝的程式庫：
 * 1. Ultrasonic by Erick Simões
 * 2. PubSubClient
 * 3. ArduinoJson
 */
...
// ==================== 進階設定 ====================
const unsigned long SEND_INTERVAL = 20;      // 發送間隔 (毫秒)
...
void setup() {
  ...
  Serial.println("\n════════════════════════════════════════");
  Serial.println("ESP32 HC-SR04 (Ultrasonic.h) MQTT 發送器");
  Serial.println("════════════════════════════════════════");
  ...
  Serial.println(" (Echo)");
  Serial.println("✅ 使用 Ultrasonic.h 程式庫");
  Serial.println();
  
  // 測試感測器
  Serial.println("測試感測器連接...");
  for (int i = 0; i < 3; i++) {
    long testDist = ultrasonic.read();
    Serial.print("  測試 #");
    Serial.print(i + 1);
    Serial.print(": ");
    if (testDist <= 0 || testDist > MAX_DISTANCE) {
      Serial.println("❌ 無讀取");
    } else {
